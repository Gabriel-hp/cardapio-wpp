    ...
    cardapio: [
        {
            ...
            name: "saladaaaa com cxxfsfs",
            dsc: "saladaaaa, saladaaaasaladaaaaEnds",
            price: 10.99
...
// CREAT
function criaMen(dados){ 
    MEN.cardapio.push({
        id: MEN.cardapio.length + 1 ,
        name: dados.name,
        dsc: dados.dsc,
        price: dados.price
    });
}
ciarCardapio({id:'', name:'', dsc:'', price:''});
console.log(MEN.cardapio) [ {id:'x-salad', name:'x-salada', dsc:'xahjakjahka', price:7.50 } ]
// READ
function pegarMen() {
    return MEN.cardapio;
}
// UPDATE
function atualizarConteudodoMen(id, novoMen){
   const carpadioatualizado = pegarMen().find((post) => {
        return post.id === id;
    })
    console.log(carpadioatualizado) 
}
atualizarConteudodoMen(1, 'novo cont')
// DELETE
